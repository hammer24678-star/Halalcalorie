...
class _BodyScreenState extends ConsumerState<BodyScreen> with SingleTickerProviderStateMixin {
      ...
      Container(
        ...
        decoration: BoxDecoration(
          gradient: LinearGradient(
            colors: p.isMale
              ? [AppColors.brandGreen, AppColors.darkGreen]
              : [AppColors.accentGold, const Color(0xFFB8860B)],
  ...
  Widget _buildDetails(UserProfile p, bool isPremium, bool isAr, bool isDark) {
    ...
    return ListView(padding: const EdgeInsets.all(14), children: [
      ...
      _detailRow(tLang(lang, 'معدل الأيض الأساسي (BMR)', 'Basal Metabolic Rate (BMR)', 'Métabolisme de base (MB)', 'Bazal Metabolizma Hızı (BMH)', 'Kadar Metabolisme Basal (BMR)', 'Laju Metabolisme Basal (BMR)'),
          '${p.bmrKcal.toInt()} kcal', AppColors.brandGreen,
      ...
      _detailRow(tLang(lang, 'هدف السعرات الموصى به', 'Recommended Calorie Goal', 'Objectif calorique recommandé', 'Önerilen Kalori Hedefi', 'Sasaran Kalori Disyorkan', 'Target Kalori yang Direkomendasikan'),
          '${p.calorieGoalKcal.toInt()} kcal', AppColors.accentGold,
      ...
      _detailRow(tLang(lang, 'الوزن المثالي (Devine)', 'Ideal Weight (Devine)', 'Poids idéal (Devine)', 'İdeal Ağırlık (Devine)', 'Berat Ideal (Devine)', 'Berat Ideal (Devine)'),
          '${p.idealWeightKg.toStringAsFixed(1)} kg', AppColors.brandGreen, '', cardBg, textC, muted),
    ...
    return ListView(padding: const EdgeInsets.all(14), children: [
      ...
      Container(
        ...
        child: Column(children: [
          ...
          const SizedBox(height: 12),
          _macroRow(tLang(lang, '🧈 دهون صحية', '🧈 Healthy Fats', '🧈 Graisses saines', '🧈 Sağlıklı Yağlar', '🧈 Lemak Sihat', '🧈 Lemak Sehat'), p.fatGrams, p.calorieGoalKcal / 9 * 0.4, AppColors.accentGold, isAr),
          const SizedBox(height: 16),
          Container(
              ...
              LineChartBarData(
                ...
                dotData: FlDotData(
                  ...
                  getDotPainter: (spot, _, __, ___) => FlDotCirclePainter(
                    radius: 4, color: AppColors.brandGreen,
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          gradient: LinearGradient(
            colors: [
              (isPremium ? AppColors.accentGold : AppColors.brandGreen).withOpacity(0.15),
              (isPremium ? AppColors.accentGold : AppColors.brandGreen).withOpacity(0.05),
            ],
          ),
          border: Border.all(
            color: (isPremium ? AppColors.accentGold : AppColors.brandGreen).withOpacity(0.6),
            width: 1.5,
          ),
          borderRadius: BorderRadius.circular(16),
        ),
  ...
  String _bmiAdvice(UserProfile p, bool isAr) {
    ...
    if (isAr) {
      ...
      if (bmi < 25)   return '✅ Within the reference range. Keep the habits that got you here.';
