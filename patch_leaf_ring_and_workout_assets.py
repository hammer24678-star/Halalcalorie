# patch_leaf_ring_and_workout_assets.py
#
# Remaster of the Nutrition "Today" ring + first pass at workout-muscle
# art, per the Sep 2026 request:
#   1. Replace the plain CircularProgressIndicator calorie ring with a
#      detailed animated leaf (lib/features/nutrition/widgets/
#      leaf_progress_ring.dart) that also carries protein/carbs/fat as
#      three droplets on its midrib, and re-skins for light/dark mode
#      and for Ramadan mode (gold + crescent instead of green).
#   2. Wire the new user-supplied muscle-illustration PNGs into the
#      workout player screen (one per workout category).
#   3. Register the new asset folders (assets/emoji/, assets/muscles/)
#      in pubspec.yaml so the emoji packs are also available to draw on
#      later (this patch does not yet wire emoji into any screen —
#      only the ring and the muscle art have call sites so far).
#
# Run this AFTER extracting halalcalorie_new_assets.zip into the
# project root (it adds assets/emoji/**, assets/muscles/**, and does
# NOT touch code) — this script only edits/creates .dart and .yaml
# files, it does not copy images.
#
# IMPORTANT — licensing note, read before shipping a store build:
#   - The Ronald-McDonald-branded emoji pack, and every McDonald's/
#     Wendy's/KFC/Popeyes-branded file inside the fast-food pack, were
#     LEFT OUT of assets/emoji/ on purpose (see
#     assets/emoji/EXCLUDED_ITEMS.txt for the exact list) — those are
#     other companies' trademarks/mascots and don't belong in a
#     monetized halal-nutrition app regardless of where they were
#     downloaded from.
#   - The four muscle reference sheets you sent have a faint tiled
#     watermark baked into their (fake, flattened) checkerboard
#     background — that's the signature of a stock-site PREVIEW
#     download, not a licensed asset. The individual PNGs under
#     assets/muscles/ were auto-cropped from those sheets with real
#     alpha transparency so they're usable as placeholders right now,
#     but swap them for art you've actually licensed (or commissioned/
#     drew yourself) before this reaches a store build.
#
...
def write_new_file(rel_path, content, label, marker):
    """Idempotent file creation: if the file already exists and already
    contains `marker`, skip (already applied by a previous run). If it
    exists WITHOUT the marker, refuse rather than clobber unknown
    content someone else put there."""
    p = ROOT / rel_path
    if p.exists():
        ...
        if marker in existing:
            _log(label, "SKIPPED-ALREADY")
            return
        raise SystemExit(f"ERROR ({label}): {rel_path} already exists and doesn't "
                          f"contain the expected marker -- refusing to overwrite.")
...
FITNESS_SCREEN = 'lib/features/fitness/fitness_screen.dart'
MUSCLE_ASSETS = 'lib/data/muscle_assets.dart'
PUBSPEC = 'pubspec.yaml'


# -----------------------------------------------------------------------
# 1. New widget file: the leaf itself.
# -----------------------------------------------------------------------

LEAF_WIDGET_MARKER = "PATCH_LEAF_RING_AND_WORKOUT_ASSETS"
...
def patch_leaf_widget_file():
    write_new_file(LEAF_WIDGET, LEAF_WIDGET_SOURCE,
                    'create leaf_progress_ring.dart', LEAF_WIDGET_MARKER)


# -----------------------------------------------------------------------
# 2. New data file: category -> muscle asset map.
# -----------------------------------------------------------------------

MUSCLE_ASSETS_SOURCE = r'''// muscle_assets.dart
// PATCH_LEAF_RING_AND_WORKOUT_ASSETS
//
...
def patch_muscle_assets_file():
    write_new_file(MUSCLE_ASSETS, MUSCLE_ASSETS_SOURCE,
                    'create muscle_assets.dart', LEAF_WIDGET_MARKER)


# -----------------------------------------------------------------------
# 3. pubspec.yaml -- register the new asset folders. Flutter does NOT
#    recurse into subfolders of a listed asset directory, so each emoji
#    subfolder needs its own line.
# -----------------------------------------------------------------------
...
PUBSPEC_NEW = """  assets:
    ...
    - assets/images/
    - assets/muscles/
    - assets/emoji/food_drink/
    - assets/emoji/cupcakes/
    - assets/emoji/clipart_foods/
    - assets/emoji/fast_food/
    - assets/emoji/face_emojis/
    - assets/emoji/blex_emotes/
    - assets/store/"""


def patch_pubspec():
    apply_literal(PUBSPEC, PUBSPEC_OLD, PUBSPEC_NEW,
                  'pubspec.yaml: register assets/muscles + assets/emoji/*',
                  skip_if="assets/muscles/")


# -----------------------------------------------------------------------
# 4. nutrition_screen.dart -- import the widget, expose isRamadan,
#    stop clamping pct at 1.0 (the leaf needs to know when it's over
#    goal, same threshold calCol already uses), and swap the
#    CircularProgressIndicator Stack for LeafProgressRing.
# -----------------------------------------------------------------------

NUTRITION_HUNKS = [
    (
        "import leaf_progress_ring.dart",
    ...
    (
        "stop clamping pct so the leaf can show 'over goal'",
        "    final pct   = goal > 0 ? (eaten / goal).clamp(0.0, 1.0) : 0.0;",
        "    final pct   = goal > 0 ? eaten / goal : 0.0;",
    ),
    (
        "calorie ring -> LeafProgressRing",
        """                          // Calorie ring
...
def patch_nutrition_screen():
    for (label, old, new), marker in zip(NUTRITION_HUNKS, NUTRITION_SKIP_MARKERS):
        apply_literal(NUTRITION_SCREEN, old, new,
                      f'nutrition_screen.dart: {label}', skip_if=marker)


# -----------------------------------------------------------------------
# 5. fitness_screen.dart -- import the muscle map and show the relevant
#    illustration above the emoji on the workout player screen.
# -----------------------------------------------------------------------
...
FITNESS_HUNKS = [
    (
        ...
        "import'../../data/models/models.dart';\n\n"
        "// ══════════════════════════════════════════════════",
        "import'../../data/models/models.dart'; import '../../data/muscle_assets.dart';\n\n"
        "// ══════════════════════════════════════════════════",
    ),
    (
        "muscle illustration above the workout-player emoji",
        """          // ── Emoji + step info ─────────────────────────────
...
def patch_fitness_screen():
    for (label, old, new), marker in zip(FITNESS_HUNKS, FITNESS_SKIP_MARKERS):
        apply_literal(FITNESS_SCREEN, old, new,
                      f'fitness_screen.dart: {label}', skip_if=marker)


def main():
    patch_leaf_widget_file()
    patch_muscle_assets_file()
    patch_pubspec()
    patch_nutrition_screen()
    patch_fitness_screen()

    print("\n=== leaf-ring-and-workout-assets ledger ===")
