#!/usr/bin/env python3
"""
patch_v37_icon_overhaul_and_nutrition_barcode.py
=================================================

v37 CHANGE (why you're seeing this file instead of v36):
Your v36 run logged "SKIPPED-NOT-FOUND" for both code edits on every
single attempt -- from ~, from ~/HalalCalorie, and from inside
halalcalorie-repo itself (where Step 2 proved ROOT was correct, since
the icon PRESENT checks resolved fine from that same ROOT). That's the
tell: "SKIPPED-NOT-FOUND" in v36 meant two *different* things collapsed
into one label --
  (a) the target file genuinely doesn't exist at that relative path, or
  (b) the file exists, but neither the old-text anchor nor the new-text
      already matches -- i.e. the file's real content has drifted from
      what this patch expects.
v36 couldn't tell you which one you were looking at. This version can:
it now searches for MODELS/NUTRITION_SCREEN by filename anywhere under
ROOT if the hardcoded path is wrong, and if the file IS found but the
anchor still doesn't match, it prints the actual current line(s) near
the expected spot so you can see exactly what changed and why.
Nothing about steps 2-4 (icon generation, background stripping,
barcode action) is touched -- same logic as v36.
...
LEDGER = []
MARKER = "PATCH_V37_ICON_OVERHAUL"
...
def _locate(rel: str) -> Path:
    """Resolve rel against ROOT. If that exact path is missing, fall back
    to searching for a file with the same name anywhere under ROOT (skipping
    build/ .dart_tool/ .git so we don't match generated or vendored copies).
    Returns the hardcoded path unchanged if no better candidate is found,
    so downstream logging can still say NOT-FOUND accurately."""
    direct = ROOT / rel
    if direct.exists():
        return direct
    name = Path(rel).name
    skip = {"build", ".dart_tool", ".git", ".gradle", "ios", "android"}
    matches = [
        p for p in ROOT.rglob(name)
        if not skip & set(p.relative_to(ROOT).parts)
    ]
    if len(matches) == 1:
        print(f"  NOTE: {rel} not at expected path -- using {matches[0].relative_to(ROOT)} instead")
        return matches[0]
    if len(matches) > 1:
        print(f"  NOTE: {rel} -- {len(matches)} files named {name} found, "
              f"none at the expected path; refusing to guess which one:")
        for m in matches:
...
def _fingerprint_lines(text, old, context=1):
    """When the exact anchor can't be found, locate the most distinctive
    short token from `old` (its longest single line) and print any lines
    in the live file containing that token, so a stale anchor is visible
    instead of a silent miss."""
    candidate_lines = [ln.strip() for ln in old.splitlines() if ln.strip()]
    if not candidate_lines:
        return
    token_line = max(candidate_lines, key=len)
    token = token_line[:40]
    hits = [(i, ln) for i, ln in enumerate(text.splitlines()) if token[:20] in ln]
    if not hits:
        # fall back to a shorter, more forgiving fragment
        short_token = token_line.split(",")[0][:24]
        hits = [(i, ln) for i, ln in enumerate(text.splitlines()) if short_token in ln]
    if hits:
        print(f"      current file has {len(hits)} near-match line(s) instead:")
        for i, ln in hits[:3]:
            print(f"        L{i + 1}: {ln.strip()[:110]}")
    else:
        print(f"      no line in the file resembles: {token[:60]}...")


def edit(rel, old, new, label):
    p = _locate(rel)
    if not p.exists():
        _log(label, "FILE-NOT-FOUND")
        return False
    ...
    if old not in text:
        if new in text:
            ...
            _log(label, "ANCHOR-MISMATCH (file exists, expected text not there)")
            _fingerprint_lines(text, old)
        return False
